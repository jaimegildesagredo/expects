...
with describe('start_with'):
    ...
    with it('should_pass_if_string_starts_with_string'):
        expect(self.str).to(start_with(self.str[:5]))

    with it('should_pass_if_list_starts_with_arg'):
        expect(self.lst).to(start_with(self.lst[0]))

    with it('should_pass_if_list_starts_with_args'):
        expect(self.lst).to(start_with(*self.lst[:2]))

    with it('should_pass_if_ordered_dict_starts_with_keys'):
        expected_args = list(self.ordered_dct)[:2]

        expect(self.ordered_dct).to(start_with(*expected_args))

    with it('should_pass_if_iter_starts_with_args'):
        expect(iter(self.lst)).to(start_with(*self.lst[:2]))

    with it('should_fail_if_string_does_not_start_with_string'):
        expected = self.str[5:]

        with failure('to start with {!r}'.format(expected)):
            expect(self.str).to(start_with(expected))

    with it('should_fail_if_list_does_not_start_with_arg'):
        expected = self.lst[1]

        with failure('to start with {!r}'.format(expected)):
            expect(self.lst).to(start_with(expected))

    with it('should_fail_if_list_does_not_start_with_args'):
    ...
    with it('should_fail_if_list_starts_with_first_arg_but_not_second'):
        expected_args = self.lst[0], self.lst[0]

        with failure('to start with {!r} and {!r}'.format(*expected_args)):
            expect(self.lst).to(start_with(*expected_args))

    with it('should_fail_if_actual_is_a_dict'):
    ...
    with context('#negated'):
        with it('should_pass_if_string_does_not_start_with_string'):
            expect(self.str).not_to(start_with(self.str[5:]))

        with it('should_pass_if_list_does_not_start_with_args'):
            expect(self.lst).not_to(start_with(*self.lst[2:]))

        with it('should_pass_if_list_starts_with_first_arg_but_not_second'):
            expected_args = self.lst[0], self.lst[0]
